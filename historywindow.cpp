...
void historywindow::replayNextMove()
{
    ...
    if (session.gameMode == "Player vs Player") {
        ...
        if (session.outcome.contains("Player wins")) {
            // Player won, their symbol is winnerSymbol
            symbol = (currentMoveIndex % 2 == 0) ? session.winnerSymbol : (session.winnerSymbol == "X" ? "O" : "X");
        } else if (session.outcome.contains("AI wins")) {
            // AI won, their symbol is winnerSymbol
            symbol = (currentMoveIndex % 2 == 0) ? (session.winnerSymbol == "X" ? "O" : "X") : session.winnerSymbol;
        } else {
            // Tie or no winner, assume player starts with 'O' (default from GameBoard)
            symbol = (currentMoveIndex % 2 == 0) ? "O" : "X";
        }
    }
